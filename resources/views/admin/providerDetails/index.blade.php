                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <small class="text-muted">{{ __('messages.Phone') }}:
                                {{ $provider->country_code }}{{ $provider->phone }}</small>
                        </div>
                        <div>
                                    ...
                                    @forelse($providerTypes as $providerType)
                                        <tr>
                                            <td>
                                                @if ($providerType->images->first())
                                                    <img src="{{ $providerType->images->first()->photo_url }}"
                                                        alt="{{ $providerType->name }}" class="img-thumbnail"
                                            ...
                                            <td>
                                                ...
                                                <br>
                                                <small
                                                    class="text-muted">{{ Str::limit($providerType->description, 50) }}</small>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                @if (isset($providerType->type->booking_type) && $providerType->type->booking_type == 'service')
                                                    ...
                                                    @if ($serviceCount > 0)
                                                        <br>
                                                        <small
                                                            class="text-muted">{{ __('messages.Service_Based_Pricing') }}</small>
                                                    @endif
                                                @else
                                            ...
                                            <td>
                                                @if (!isset($providerType->type->booking_type) || $providerType->type->booking_type == 'hourly')
                                                    {{ number_format($providerType->price_per_hour, 2) }}
                                                    {{ __('messages.Currency') }}
                                                @else
                                                    <span
                                                        class="text-muted">{{ __('messages.Service_Based_Pricing') }}</span>
                                                @endif
                                            </td>
                                            <td>
                                                <span
                                                    class="badge {{ $providerType->status == 1 ? 'bg-success' : 'bg-danger' }}">
                                                    {{ $providerType->status == 1 ? __('messages.On') : __('messages.Off') }}
                                                </span>
                                                <br>
                                                <span
                                                    class="badge {{ $providerType->activate == 1 ? 'bg-success' : 'bg-warning' }}">
                                                    {{ $providerType->activate == 1 ? __('messages.Active') : __('messages.Inactive') }}
                                                </span>
                                            </td>
                                            <td>
                                                <span
                                                    class="badge {{ $providerType->is_vip ? 'bg-warning text-dark' : 'bg-secondary' }}">
                                                    {{ $providerType->is_vip ? __('messages.VIP') : __('messages.Regular') }}
                                                </span>
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn-group-vertical btn-group-sm">
                                                    ...
                                                    <a href="{{ route('discounts.index', [$provider->id, $providerType->id]) }}"
                                                        ...
                                                        {{ __('messages.Manage_Discounts') }}
                                                    </a>

                                                    <form
                                                        action="{{ route('admin.providerDetails.destroy', [$provider->id, $providerType->id]) }}"

        ...
        <div class="col-md-10 offset-md-1">
            <div class="card">
                <div class="card-header">
                    <h4>{{ __('messages.Edit_Discount') }}: {{ $discount->name }}</h4>
                ...
                <div class="card-body">
                    <form action="{{ route('discounts.update', [$providerId, $providerType->id, $discount->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" class="form-control @error('percentage') is-invalid @enderror" 
                                               id="percentage" name="percentage" value="{{ old('percentage', $discount->percentage) }}" 
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3" id="service-selection" style="{{ $providerType->type->booking_type == 'service' ? 'display: block;' : 'display: none;' }}">
                                    ...
                                    <div class="border p-3 rounded @error('service_ids') border-danger @enderror" style="max-height: 250px; overflow-y: auto;">
                                        <div class="mb-2 border-bottom pb-2">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" id="all_services" 
                                                       {{ empty($selectedServiceIds) ? 'checked' : '' }} onchange="toggleAllServices()">
                                        ...
                                        <div id="specific-services" style="{{ empty($selectedServiceIds) ? 'display: none;' : '' }}">
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="alert {{ $discount->isCurrentlyActive() ? 'alert-success' : 'alert-warning' }}">
                                    <h6><i class="fas fa-info-circle"></i> {{ __('messages.Current_Status') }}</h6>
                                    @if($discount->isCurrentlyActive())
                                        <p class="mb-0">{{ __('messages.This_discount_is_currently_active') }}</p>
                                    @elseif($discount->start_date > now())
                                        <p class="mb-0">{{ __('messages.This_discount_will_start_on') }} {{ $discount->start_date->format('Y-m-d') }}</p>
                                    @else
                                        <p class="mb-0">{{ __('messages.This_discount_has_expired') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mt-2">
                            <div class="col-md-12">
                                <div class="alert alert-info">
                                    <h6><i class="fas fa-eye"></i> {{ __('messages.Discount_Preview') }}</h6>
                                    <div id="discount-preview">
                                        {{ __('messages.Loading_preview') }}
                                    </div>
                                </div>
                            </div>
                        </div>

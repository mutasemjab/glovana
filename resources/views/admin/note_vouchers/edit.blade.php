...
@section('title', __('messages.edit_note_voucher'))
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ __('messages.edit_note_voucher') }} #{{ $noteVoucher->number }}</h3>
                    <a href="{{ route('note-vouchers.index') }}" class="btn btn-secondary float-end">
                        <i class="fas fa-arrow-left"></i> {{ __('messages.back') }}
                    </a>
                </div>
                <div class="card-body">
                    <form action="{{ route('note-vouchers.update', $noteVoucher) }}" method="POST" id="voucherForm">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" 
                                           ...
                                           id="number" 
                                           name="number" 
                                           value="{{ old('number', $noteVoucher->number) }}" 
                                           required>
                                    @error('number')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('type') is-invalid @enderror" 
                                        ...
                                        <option value="1" {{ old('type', $noteVoucher->type) == '1' ? 'selected' : '' }}>
                                            {{ __('messages.in') }}
                                        </option>
                                        <option value="2" {{ old('type', $noteVoucher->type) == '2' ? 'selected' : '' }}>
                                            {{ __('messages.out') }}
                                        </option>
                                    </select>
                                    @error('type')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" 
                                           ...
                                           value="{{ old('date_note_voucher', $noteVoucher->date_note_voucher->format('Y-m-d')) }}" 
                                           required>
                                    ...
                                    <select class="form-control @error('warehouse_id') is-invalid @enderror" 
                                        ...
                                        @foreach ($warehouses as $warehouse)
                                            <option value="{{ $warehouse->id }}" 
                                                    {{ old('warehouse_id', $noteVoucher->warehouse_id) == $warehouse->id ? 'selected' : '' }}>
                                                {{ $warehouse->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-control @error('order_id') is-invalid @enderror" 
                                        ...
                                        @foreach ($orders as $order)
                                            <option value="{{ $order->id }}" 
                                                    {{ old('order_id', $noteVoucher->order_id) == $order->id ? 'selected' : '' }}>
                                                {{ $order->id }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('note') is-invalid @enderror" 
                                              id="note" 
                                              name="note" 
                                              rows="3">{{ old('note', $noteVoucher->note) }}</textarea>
                                    @error('note')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div id="products-container">
                            @foreach($noteVoucher->voucherProducts as $index => $voucherProduct)
...
document.addEventListener('DOMContentLoaded', function() {
    let productIndex = {{ $noteVoucher->voucherProducts->count() }};

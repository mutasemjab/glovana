...
@section('title')
{{ __('messages.add_vip_subscription') }}
@endsection
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">{{ __('messages.add_new_subscription') }}</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.vip-subscriptions.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="provider_type_id" id="provider_type_id" class="form-control @error('provider_type_id') is-invalid @enderror" required>
                            ...
                            @foreach($providerTypes as $providerType)
                                <option value="{{ $providerType->id }}" {{ old('provider_type_id') == $providerType->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="amount_paid" id="amount_paid" 
                               ...
                               value="{{ old('amount_paid') }}" step="0.01" min="0" required>
                        @error('amount_paid')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="payment_status" id="payment_status" class="form-control @error('payment_status') is-invalid @enderror" required>
                                    <option value="1" {{ old('payment_status', '1') == '1' ? 'selected' : '' }}>{{ __('messages.paid') }}</option>
                                    <option value="2" {{ old('payment_status') == '2' ? 'selected' : '' }}>{{ __('messages.unpaid') }}</option>
                                </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="payment_method" id="payment_method" class="form-control @error('payment_method') is-invalid @enderror">
                            ...
                            <option value="wallet" {{ old('payment_method') == 'wallet' ? 'selected' : '' }}>{{ __('messages.wallet') }}</option>
